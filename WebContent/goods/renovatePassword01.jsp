...
<html>
	...
	<script type="text/javascript">
		...
		function check(){
			if($(".error").length!=0){
				window.wxc.xcConfirm("邮箱或验证码不正确！", window.wxc.xcConfirm.typeEnum.error);
				return false;
			}else{
				return true;
			}
				...
				if(mailbox.length!=0){
					$.get("../MailBoxCheckServlet?mailbox="+mailbox,function(data,status){
						if(data=="false"){
							...
							$(".mailbox").after("<span class='icon-checkmark' style='color: green;'></span><span style='color: green;'> 邮箱存在</span>");
							...
							if(!regex.test(mailbox)){
								...
								$(".mailbox").siblings().remove();
								$(".mailbox").after("<span class='icon-cross error' style='color: red;'></span><span style='color: red;'> 邮箱不存在</span>");
							}
						}
					});
				}else{
	...
	<body style="background-color: rgb(240, 243, 239);">
		...
		<div class="renovatePassword">
			<div align="center" style="margin: 20px 0;">
				...
				<div class="index"><h4>2</h4><h4>修改密码</h4></div><span>······></span>
				<div class="index"><h4>3</h4><h4>修改成功</h4></div>
			</div>
			<form action="../RenovatePasswordServlet" method="get"  onsubmit="return check()">
